...
import com.easy.demo.net.R;
import com.easy.demo.net.easy.exception.ResultException;
import com.easy.demo.net.easy.model.HttpCacheManager;
...
public final class RequestHandler implements IRequestHandler {
    ...
    public RequestHandler() {
        mApplication = FanAppContext.getApplication();
    }
    ...
    public Object requestSuccess(@NonNull HttpRequest<?> httpRequest, @NonNull Response response, @NonNull Type type) throws Throwable {
        ...
        if (!response.isSuccessful()) {
            throw new ResponseException(mApplication.getString(R.string.com_net_error), response);
        }
...
//            throw new DataException(mApplication.getString(R.string.http_data_explain_error), e);
            throw new DataException("data explain error", e);
        }
...
//            return new TimeoutException(mApplication.getString(R.string.http_server_out_time), throwable);
            return new TimeoutException("http server out time", throwable);
        }
...
//                return new ServerException(mApplication.getString(R.string.http_server_error), throwable);
                return new ServerException("http server out time", throwable);
            }
            // 没有连接就是网络异常
//            return new NetworkException(mApplication.getString(R.string.http_network_error), throwable);
            return new NetworkException("http server out time", throwable);
        }
...
//            return new CancelException(mApplication.getString(R.string.http_request_cancel), throwable);
            return new CancelException("http request cancel", throwable);
        }
...
//            responseException.setMessage(String.format(mApplication.getString(R.string.http_response_error), response.code(), response.message()));
            responseException.setMessage("http_response_error " + response.code() + " " + response.message());
...
//            nullBodyException.setMessage(mApplication.getString(R.string.http_response_null_body));
            nullBodyException.setMessage("http_response_null_body ");
...
//            fileMd5Exception.setMessage(mApplication.getString(R.string.http_response_md5_error));
            fileMd5Exception.setMessage("http_response_md5_error ");
